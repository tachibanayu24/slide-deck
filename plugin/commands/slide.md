...
テーマを受け取ったら、AskUserQuestion で以下を **1回で** 確認します。推測できるものはデフォルト値を示し、変更がなければそのまま進めます：

```
以下の内容でスライドを作成します。変更があれば教えてください。

- 資料の種類: [推測]
- トーン: [推測]
- 対象: [推測]
- 枚数: [推測]
- コピーライト: フッター右下に表示する文言があれば入力してください（例: `©Acme Inc.`）。不要なら空欄
- 特に入れたい要素: あれば教えてください
```
