...
#### accent-image レイアウト用の画像生成

`layout="accent-image"` で使う画像は `--scene` フラグで生成する。背景透過は不要で、縦長のリッチなシーン写真が最適:
...
slide-gen-image --scene \
  ...
  --width 300 --output [HTMLファイルのディレクトリ]/.images/[name].png
```

生成した画像は `bg-image` 属性で指定:
```html
<s-slide layout="accent-image" bg-image=".images/[name].png" header="..." message="...">
```
