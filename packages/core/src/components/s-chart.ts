...
export class SChart extends HTMLElement {
  ...
  /** Return plot-area height: user-specified `height` attribute or default CHART_HEIGHT (200). */
  private getChartHeight(): number {
    const attr = this.getAttribute("height");
    if (attr) {
      const h = parseInt(attr, 10);
      if (h > 0) return h;
    }
    return CHART_HEIGHT;
  }
    ...
    try {
      ...
      return raw.filter(
        (d: unknown): d is LineDataPoint =>
          ...
          (isFiniteNumber((d as LineDataPoint).value) ||
            Array.isArray((d as LineDataPoint).values)),
      );
    } catch {
      return [];
    }
  ...
  private renderBar(data: DataPoint[]): string {
    ...
    const chartH = this.getChartHeight();
    ...
    this.renderHGrid(parts, m, chartW, chartH, maxVal);
    ...
    data.forEach((d, i) => {
      ...
      parts.push(
        ...
        `<text x="${x + barWidth / 2}" y="${m.top + chartH + 18}" text-anchor="middle" font-size="10" fill="${this.subTextColor}">${esc(truncate(d.label, 6))}</text>`,
      );
    });
  ...
  private renderBarHorizontal(data: DataPoint[]): string {
    ...
    if (labelWidthAttr) {
      ...
      const maxRawWidth = Math.max(
        ...data.map((d) => estimateTextWidth(d.label, labelFontSize)),
      );
      ...
      truncLen = data.reduce((min, d) => {
        ...
        while (
          len > 3 &&
          estimateTextWidth(d.label.slice(0, len), labelFontSize) >
            maxLabelWidth
        ) {
          len--;
        }
  ...
  private renderLine(data: LineDataPoint[]): string {
    ...
    const svgW = m.left + chartW + m.right;
    const svgH = m.top + chartH + m.bottom + (seriesCount > 1 ? 28 : 0);
    ...
    data.forEach((d, i) => {
      ...
      parts.push(
        `<text x="${x}" y="${m.top + chartH + 18}" text-anchor="middle" font-size="10" fill="${this.subTextColor}">${esc(truncate(d.label, 6))}</text>`,
      );
    });
    ...
    for (let s = 0; s < seriesCount; s++) {
      ...
      data.forEach((d, i) => {
        ...
        coords.push({
          x: m.left + i * stepX,
          y: m.top + chartH - (val / maxVal) * chartH,
        });
      });
  ...
  private renderScatter(data: ScatterPoint[]): string {
    ...
    const groupColor = (g: string) =>
      colors[groups.indexOf(g) % colors.length] || colors[0];
    ...
    const chartW = 300;
    const svgW = m.left + chartW + m.right;
    const svgH = m.top + chartH + m.bottom + (groups.length > 0 ? 28 : 0);

    const scaleX = (v: number) =>
      m.left + ((v - minX + padX) / (rangeX + padX * 2)) * chartW;
    const scaleY = (v: number) =>
      m.top + chartH - ((v - minY + padY) / (rangeY + padY * 2)) * chartH;

    const parts: string[] = [this.svgOpen(svgW, svgH)];

    // Horizontal grid
    for (let i = 0; i <= GRID_LINES; i++) {
      const y = m.top + (chartH * i) / GRID_LINES;
    ...
    for (let i = 0; i <= GRID_LINES; i++) {
      ...
      parts.push(
        `<line x1="${x}" y1="${m.top}" x2="${x}" y2="${m.top + chartH}" stroke="${this.gridColor}" stroke-width="1" stroke-dasharray="4 4"/>`,
        `<text x="${x}" y="${m.top + chartH + 16}" text-anchor="middle" font-size="9" fill="${this.subTextColor}">${Math.round(val)}</text>`,
      );
    }
    ...
    if (xLabel) {
      parts.push(
        `<text x="${m.left + chartW / 2}" y="${m.top + chartH + 32}" text-anchor="middle" font-size="10" fill="${this.subTextColor}">${esc(xLabel)}</text>`,
      );
    }
    if (yLabel) {
      parts.push(
        `<text x="12" y="${m.top + chartH / 2}" text-anchor="middle" font-size="10" fill="${this.subTextColor}" transform="rotate(-90 12 ${m.top + chartH / 2})">${esc(yLabel)}</text>`,
      );
    }
    ...
    // Legend
    if (groups.length > 0) {
      const ly = m.top + chartH + (xLabel ? 46 : 32);
      groups.forEach((g, i) => {
